...
def common_layer_random_sample(
    layer: nn.Model, f, df, input_generator, dE_dy_generator, samples: int = 1, tolerance=1e-7, break_on_error=True
):
    checks, errors = 0, 0
    ...
    for i in range(samples):
        ...
        sample_checks, sample_errors = check_gradient_numerical(
            f, inputs, dE_dy, dE_dinputs_analytic, tolerance=tolerance, break_on_error=break_on_error
        )
...
def check_gradient_numerical(
    ...
    inputs: ParameterSet,
    dE_dy: np.ndarray,
    dE_dinputs_analytic: Dict[str, np.ndarray],
    tolerance: float,
    break_on_error: bool,
):
    errors = 0
...
def report_errors(
    dE_dk_analytical, dE_dk_numerical, label: str, x: np.ndarray, dE_dy: np.ndarray, max_error: float, tolerance: float
):
