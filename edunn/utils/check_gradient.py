...
def common_layer(l: nn.ModelWithParameters, x_shape: Tuple, samples: int = 1, tolerance=1e-7, break_on_error=True):
    ...
    common_layer_random_sample(l, f, df, input_generators, δEδy_generator, samples=samples, tolerance=tolerance,
...
def squared_error(l: nn.SquaredError, y_shape: Tuple, samples: int = 1, tolerance=1e-7, break_on_error=True):
    ...
    common_layer_random_sample(l, f, df_ignore_δEδy, input_generators, δE_generator, samples=samples,
...
def common_layer_random_sample(l: nn.Model, f, df, input_generator, δEδy_generator, samples: int = 1, tolerance=1e-7,
                               break_on_error=True):
    checks, errors = 0, 0
    ...
    for i in range(samples):
        ...
        sample_checks, sample_errors = check_gradient_numerical(f, inputs, δEδy, δEδinputs_analytic,
                                                                tolerance=tolerance, break_on_error=break_on_error)
...
def check_gradient_numerical(f: Callable, inputs: ParameterSet, δEδy: np.ndarray,
                             δEδinputs_analytic: Dict[str, np.ndarray], tolerance: float, break_on_error: bool):
    errors = 0
    checks = 0
...
def report_errors(δEδk_analytical, δEδk_numerical, label: str, x: np.ndarray, δEδy: np.ndarray, max_error: float,
                  tolerance: float):
