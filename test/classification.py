...
def train_and_test(dataset_name:str, model_generator:Callable, epochs:int, lr:float):
...
def test_classification_model(model_generator, datasets_config):

    for dataset_name,config in datasets_config.items():
        lr,epochs,min_accuracy = config.lr,config.epochs,config.min_accuracy
        model,accuracy=train_and_test(dataset_name, model_generator, epochs, lr)
...
test_classification_model(network2, config_datasets)